...
function buildPlans(goal, showAll = false) {
  plansGrid.innerHTML = '';
  const filtered = plans.filter((plan) => goal === 'maintain' ? plan.goal !== 'loss' : plan.goal === goal || plan.goal === 'loss');
  ...
  visible.forEach((plan) => {
    ...
    card.innerHTML = `
      ...
      <div class="price-row">
        ...
        <a class="button" href="#promo-lead" data-cta="plan">Подобрать рацион</a>
      </div>
    `;
    plansGrid.appendChild(card);
  });
  showMoreBtn.hidden = filtered.length <= visible.length;
}
...
function bindCtaTracking() {
  document.addEventListener('click', (event) => {
    ...
    if (target instanceof HTMLElement && target.closest('.button')) {
      ...
      trackEvent('cta_click', { label: button.textContent?.trim() || 'CTA' });
    }
  });
}
...
function handleSubmit(event) {
  ...
  lastGoal = goal;
  buildPlans(goal);
  trackEvent('calculator_submit', { goal });
  trackEvent('form_submit', { form: 'calculator' });
...
promoForm?.addEventListener('submit', (event) => {
  event.preventDefault();
  trackEvent('form_submit', { form: 'promo' });
});

showMoreBtn?.addEventListener('click', () => {
  showMoreBtn.hidden = true;
  buildPlans(lastGoal, true);
  trackEvent('cta_click', { label: 'Показать ещё рационы' });
});
